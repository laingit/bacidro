...
{:liv_2 "nc", :liv_2_desc "Non Classificato", :label "nc", :rgb "#DCDCDC"}
  ...
  [
   ...
   [:pdf-table
    {:header [[[:pdf-cell {:colspan 2}
                [:paragraph {:align :right :style :bold} "Customer Orders"]]]
   ...
   (into
     ...
     (for [x (range 1 10)]
       [[:cell {:color [(* 10 x) 0 0]} (str (dec x))]
        [:cell {:color [0 (* 10 x) 0]} (str x)]
        [:cell {:background-color [0 0 (* 10 x)]} (str (inc x))]]))
...
(pdf/pdf
  [{:title         "Test doc"
   ...
   #_[:svg {:under true :translate [0 270] :scale 0.95}
